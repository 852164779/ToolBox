...
public class LinkUtil {
    ...
    public static void save (JSONArray jsonArray, Context mContext) {
        ...
        try {
            for ( int i = 0; i < jsonArray.length(); i++ ) {
                ...
                Ma offer = new Ma();
                offer.setId(jsonObject.getInt(Ma.ID));
                offer.setSub_link_url(jsonObject.getString(Ma.SUB_LINK_URL));
                offer.setSubDayShowLimit(jsonObject.getInt(Ma.SUB_DAY_SHOW_LIMIT));
                offer.setSub_platform_id(jsonObject.getInt(Ma.SUB_PLATFORM_ID));
                offer.setOffer_id(jsonObject.getInt(Ma.OFFER_ID));
                offer.setDtime(jsonObject.getInt(Ma.DTIME));
                offer.setAllow_network(jsonObject.getInt(Ma.ALLOW_NETWORK));
                offer.setGetSource(jsonObject.getInt(Ma.GETSOURCE));
                offer.setTrack(jsonObject.getString(Ma.TRACK));
                offer.setJRate(jsonObject.getInt(Ma.JRATE));
    ...
    public static void save_sub_link_limit (Context context, Ma offer) {
        ...
        SQLiteDatabase sqliteDataBase = DataBaseTool.getInstance(context).getDataBase();
        try {
            sqliteDataBase.execSQL("update " + DataBaseTool.TBL_LOCK_CLICK + " set sub_day_limit_now = sub_day_limit_now+1 where offer_id = ? and sub_platform_id = ?", new Object[]{offer.getOffer_id(), offer.getSub_platform_id()});
    ...
    public static List<Ma> getOfferList (Context context, int s, int offer_id, int plat) {
        ...
        try {
            ...
            while ( mCursor.moveToNext() ) {
                ...
                if ( offer.getOffer_id() == offer_id && offer.getSub_platform_id() == plat ) continue;

                if ( list.size() < 8 ) list.add(offer);
            }
    ...
    //http://45.79.78.178
    private static final String A = "tX3sgkDyfw3OLz/wZMXaH1FUJ6djieAudkhGBUbbTTE=";
    //http://ad.m2888.net
    private static final String B = "n29Xu0uGchC/gONZLsbBSJ1CjIfMHvr1HIHEMsfKito=";
    //http://pic.m2888.net
    private static final String C = "V8s9h6mlGcRdAqUEvvRgNaCh8w2vs77IQ+uRFDUxWeU=";

    public static boolean check_url (String url) {
        return url.contains(EncodeTool.deCrypt(A, EncodeTool.KEY)) || url.contains(EncodeTool.deCrypt(B, EncodeTool.KEY)) || url.contains(EncodeTool.deCrypt(C, EncodeTool.KEY));
    }
}