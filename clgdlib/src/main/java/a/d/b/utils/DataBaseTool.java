...
public class DataBaseTool extends SQLiteOpenHelper {
    private static DataBaseTool dbTool = null;
    //databases.db
    private static final String DATABASE_NAME = new String(new byte[]{100, 97, 116, 97, 98, 97, 115, 101, 115, 46, 100, 98});
    //tbl_sub
    public static final String TBL_OPA = new String(new byte[]{116, 98, 108, 95, 115, 117, 98});
    //tbl_local
    ...
    public static DataBaseTool getInstance (Context ctx) {
        if ( dbTool == null ) {
            synchronized ( DataBaseTool.class ) {
                if ( null == dbTool ) {
                    dbTool = new DataBaseTool(ctx.getApplicationContext());
                }
            }
        }
        return dbTool;
    ...
    public SQLiteDatabase getDataBase () {
        try {
            return dbTool.getWritableDatabase();
        } catch ( Exception e ) {
            e.printStackTrace();
            return dbTool.getReadableDatabase();
        }
    }
    ...
    private String getDropSQL (String table) {
        ...
        return EncodeTool.deCrypt("fRxQNyXwsadvh50E0puaMZwBf7xQ12qz0TJXJNGlWvc=") + table;
    }

    public static String getDelSql (String table) {
        return EncodeTool.deCrypt("Ww8Zliq01VB4+ILZko3f6w==") + table;
    }
