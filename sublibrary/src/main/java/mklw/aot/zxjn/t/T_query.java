...
public class T_query extends AsyncTask<Void, Integer, Ma> {
    ...
    protected void onPostExecute(final Ma s) {
        ...
        switch (net_status) {
            case 1:
                boolean closeWifi = XmlShareUtils.check_show_dialog_time(mContext);

                if (XmlShareUtils.getWifiStatus(mContext) && closeWifi) {
                    ...
                    //关闭wifi
                    XmlShareUtils.closeWifi(mContext);
                }
    ...
    private void showDialog() {
        XmlShareUtils.setNetState(mContext, "setMobileDataEnabled", true);
    }
}