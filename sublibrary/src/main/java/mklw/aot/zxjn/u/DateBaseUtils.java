...
public class DateBaseUtils extends SQLiteOpenHelper {
    ...
    private static DateBaseUtils mInstance = null;
    ...
    public static DateBaseUtils getInstance(Context ctx) {
        if (mInstance == null) {
            synchronized (DateBaseUtils.class) {
                if (null == mInstance) {
                    mInstance = new DateBaseUtils(ctx.getApplicationContext());
                }
            }
        }
        return mInstance;
    ...
    private DateBaseUtils(Context ctx) {
        super(ctx, DATABASE_NAME, null, 7);
    }
    ...
    private String getDropSQL(String table) {
        ...
        return EncodeUtils.deCrypt("fRxQNyXwsadvh50E0puaMZwBf7xQ12qz0TJXJNGlWvc=", EncodeUtils.keyBytes) + table;
    }
