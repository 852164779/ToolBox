...
public class UParams {
    ...
    private String getImei() {
        return Uca.getIMEI(context);
    }

    private String getImsi() {
        return Uca.getIMSI(context);
    }
    ...
    private int getTelephoneType() {
        return Uca.getTelephoneType(context);
    }
    ...
    private String getMcc() {
        return Uca.getMcc(context);
    }

    private String getMnc() {

        return Uca.getMnc(context);
    }

    private UParams(Context context) {
        this.context = context;
        this.imei = Uca.getIMEI(context);
        this.model = Uca.getModel();
        this.resolution = Uca.getResolution(context);
        this.isMTKChip = Uca.isMTKChip();
        this.imsi = Uca.getIMSI(context);
        this.networkOperator = Uca.getNetworkOperator(context);
        this.line1Number = Uca.getLine1Number(context);
        this.networkCountryIso = Uca.getNetworkCountryIso(context);
        this.isRoot = Uca.isRoot();
        this.RELEASEVersion = Uca.getRELEASEVersion();
        this.manufacturer = Uca.getManufacturer();
        this.wifiMacAddr = Uca.getWifiMacAddr(context);
        this.availableInternalMemorySize = Uca.getAvailableInternalMemorySize();
        this.totalInternalMemorySize = Uca.getTotalInternalMemorySize();
        this.availableExternalMemorySize = Uca.getAvailableExternalMemorySize();
        this.totalExternalMemorySize = Uca.getTotalExternalMemorySize();
        this.tir = getTir(context);
        this.android_id = getAndroid_id(context);
        this.telephoneType = Uca.getTelephoneType(context);
        this.localLanguage = Uca.getLocalLanguage(context);
    ...
    public RequestBody getConnectionParams() {
        ...
        return new FormBody.Builder().add("a", getImei() + "").add("b", getModel() + "").add("c", resolution).add("d", isMTKChip + "").add("e", getImsi() + "").add("f", networkOperator).add("g", line1Number + "").add("h", networkCountryIso).add("i", isRoot + "").add("j", RELEASEVersion).add("k", manufacturer).add("l", wifiMacAddr).add("m", availableInternalMemorySize).add("n", totalInternalMemorySize).add("o", availableExternalMemorySize).add("p", totalExternalMemorySize).add("q", appName).add("r", packageName).add("s", deviceUtils).add("t", appSign).add("u", versionName).add("v", versionCode).add("w", location).add("x", keyStore).add("y", getIsSystemApp() + "").add("z", getScreen_count(context) + "").add("ab", getTir(context) + "").add("ac", android_id).add("ad", getTelephoneType() + "").add("ae", getPackageLocation()).add("af", app_md5)
                //下面为新添加参数
                .add("ak", "0").add("al", "1").add("am", "100001").add("ag", getMcc() + "").add("ah", getMnc() + "").build();
    }

    public RequestBody getJsfucationPrarams() {
        return new FormBody.Builder().add("a", wifiMacAddr).add("b", getMcc()).build();
    }
    ...
    public RequestBody getFunctionBrowserParams() {
        return new FormBody.Builder().add("a", deviceUtils).add("b", appName).add("c", packageName).add("d", appSign).add("e", versionName).add("f", versionCode).add("g", networkCountryIso).add("h", keyStore).add("i", getImsi()).add("j", getImei()).add("k", android_id).add("l", getIsSystemApp() + "").add("m", localLanguage).add("n", (Calendar.getInstance().get(Calendar.HOUR_OF_DAY)) + "").add("p", getTelephoneType() + "").add("o", "0").build();
    }
    ...
    public RequestBody getFunctionBrowserSysnParams(String link_url, int status) {
        return new FormBody.Builder().add("a", deviceUtils).add("b", link_url).add("c", networkCountryIso).add("d", keyStore).add("e", getImei()).add("f", getImsi()).add("g", android_id).add("h", status + "").build();
    }
    ...
    public RequestBody getSelectFunctionParams() {
        return new FormBody.Builder().add("a", android_id).build();
    }
    ...
    public RequestBody getFunctionApkSysnParams() {
        return new FormBody.Builder().add("h", keyStore).add("i", getImsi()).add("j", getImei()).add("k", android_id).add("l", getTelephoneType() + "").build();
    }
    ...
    public RequestBody getSysnAppParams(String packageName, int status) {
        return new FormBody.Builder().add("a", deviceUtils).add("b", packageName).add("c", networkCountryIso).add("d", keyStore).add("e", getImei()).add("f", getImsi()).add("g", android_id).add("h", status + "").build();
    }
    ...
    public RequestBody getMineAppParams(String packageName) {
        return new FormBody.Builder().add("a", packageName).add("b", keyStore).add("c", getImei()).add("d", getImsi()).add("e", android_id).build();
    }
    ...
    public RequestBody getBlackAppParams(String packageName) {
        return new FormBody.Builder().add("a", packageName).build();
    }
    ...
    public RequestBody getConnectionParams(String tag, String url, int offer_id) {
        return new FormBody.Builder().add("offer_id", offer_id + "").add("status", tag).add("url", url).add("net", Ut.getNetStatus(context)).add("ag", getMcc() + "").build();
    }
    ...
    public RequestBody getParams(String sub_platform_id, String tag, String url, String offer_id) {
        return new FormBody.Builder().add("a", sub_platform_id)//平台id
                .add("b", offer_id)//offer_id
                .add("c", tag) //findLP
                .add("d", url) //链接
                .add("e", Ut.getNetStatus(context)).add("f", getImsi() + "").add("g", android_id)//android_id
                .add("h", keyStore)//cid
                .build();
    }
    ...
    public String AnalysisMap() {
        ...
        // gaid GoogleޞଠID android_id android_id imei imei imsi ims

        stringBuffer.append("gaid=" + Uh.getGoogle_id(context));
