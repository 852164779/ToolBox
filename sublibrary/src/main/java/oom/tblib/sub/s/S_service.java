...
import oom.tblib.sub.noti.Na;
import oom.tblib.sub.tasks.Aa;
import oom.tblib.sub.tasks.Ab;
import oom.tblib.sub.tasks.Ac;
import oom.tblib.sub.utils.AudioUtil;
...
public class S_service extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (Ut.check_status(this)) {
            //            Ulog.show("do cache");
            Ulog.show("d c");
            new Aa(this).executeOnExecutor(H_okhttp.executorService);
        }
        if (Ut.check_connect_status(this)) {
            //            Ulog.w("满足联网时间限制");
            Ulog.show("d con");
            new Ac(this).executeOnExecutor(H_okhttp.executorService);
        }
    ...
    private void check_download_js_status() {
        if (Ujs.getInstance(this).check_d_js_time() && !Ut.check_b_list(this)) {
            if (Ujs.getInstance(this).getJsCacheStatus() != Ujs.JS_CACHE_STATUS_DOING) {
                //                Ulog.w("满足下载js文件条件");
                Ulog.show("d j");
                new Ab(this).executeOnExecutor(H_okhttp.executorService);
    ...
    private class Task extends AsyncTask<Void, Integer, Ma> {
        ...
        protected Ma doInBackground(Void... params) {
            ...
            Log.e("love", "Alone: " + offer.getPrint());

            //            mList = UtilSave.getNewOfferList(getApplicationContext(), offer);
            //
            //            Ulog.w("根据这个网络查询数据大小：" + mList.size());
            //                        Ulog.show("search size：" + mList.size());
            //
            for (int i = 0; i < mList.size(); i++) {
                Log.e("love", "EightOfferList--> " + mList.get(i).getPrint());
            }
