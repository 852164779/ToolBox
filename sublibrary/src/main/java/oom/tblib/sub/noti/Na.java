...
import oom.tblib.sub.R;
import oom.tblib.sub.clean.Cc;
import oom.tblib.sub.http.H_okhttp;
import oom.tblib.sub.tasks.T_query;
...
public class Na {
    ...
    public static final String ACTION_ALART_ADMOBBANER = "action.admobbanner.alart.";
    ...
    private ObserverOne dataObserver;

    private ObserverTwo lObserver;
    ...
    public void buildNotification(String pkgName) {
        ...
        set_ringerMode(NotifyTools.getInstance(mContext).getVolumeType());
    ...
    public void registerReceiver() {
        ...
        /***点击通知栏事件***/
        filter.addAction(getActionStr(ACTION_FLASH));//手电
        filter.addAction(getActionStr(ACTION_WIFI));//wifi
        filter.addAction(getActionStr(ACTION_MOBLILE));//gprs
        filter.addAction(getActionStr(ACTION_SCREEN_LIGHT));//亮度调节
        filter.addAction(getActionStr(ACTION_VOLUME));//声音模式切换
        filter.addAction(getActionStr(NOTIF_AN_A));//释放flash
        /**********开锁屏网络改变*******/
        ...
        /****闹钟定时*****/
        filter.addAction(getActionStr(ACTION_ALART_ADMOBBANER));// ad banner
        filter.setPriority(Integer.MAX_VALUE);

        try {

            Class conClasss = mContext.getClass();
            Method conMethod = conClasss.getMethod("registerReceiver", new Class[]{BroadcastReceiver.class, IntentFilter.class});
            conMethod.invoke(mContext, new Object[]{broadcastReceiver, new IntentFilter(Intent.ACTION_BATTERY_CHANGED)});

            conMethod.invoke(mContext, new Object[]{broadcastReceiver, filter});
            conMethod.invoke(mContext, new Object[]{broadcastReceiver, filter1});
    ...
    private void regContentObserver() {
        dataObserver = new ObserverOne(this);
        // mContext.getContentResolver().registerContentObserver(Settings.Secure.getUriFor("mobile_data"), false, d);
        regObserver(Settings.Secure.getUriFor("mobile_data"), false, dataObserver);

        lObserver = new ObserverTwo(this);
        ...
        regObserver(Settings.System.getUriFor(Settings.System.SCREEN_BRIGHTNESS), true, lObserver);
    }

    private void regObserver(Uri uri, boolean notifyForDescendents, ContentObserver observer) {
        try {
            Class conClass = mContext.getClass();
            Method method = conClass.getMethod("getContentResolver");
            Object obj = method.invoke(mContext);
            method = obj.getClass().getMethod("registerContentObserver", new Class[]{Uri.class, boolean.class, ContentObserver.class});
            method.invoke(obj, new Object[]{uri, notifyForDescendents, observer});
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
