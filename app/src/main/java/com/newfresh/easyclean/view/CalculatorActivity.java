...
import com.newfresh.easyclean.util.CalculationUtil;
import com.newfresh.easyclean.util.EncodeTool;

import java.lang.reflect.Method;

import static com.newfresh.easyclean.MyApplication.context;

public class CalculatorActivity extends Activity {

    //setShowSoftInputOnFocus
    private static String org0 = "s8Awp7dRFhsL7qlLO40NbbEYbouElVAk9z2DKsU54k0=";
    ...
    public void onClick (View view) {
        ...
        if ( resultString.matches(context.getResources().getString(R.string.calculator_invalid_format)) ) {
            initVal();
        }
        ...
        if ( btText.matches("[0-9]|\\+|-|×|÷|\\.|(\\( \\))|=") ) {
            ...
            if ( (resultString != "") ) {
                //判断光标位置
                if ( cursorEnd ) {    //光标在尾端
                    if ( CalculationUtil.isOperator(inputString) ) {    //如果输入的是运算符，表明是继续运算
                        exp = resultString;
                        resultString = "";
                    } else if ( inputString.matches("=") ) {    //若输入"="，继续重复上一次的运算符运算
                        exp = exp.replaceAll("\n=.*", "");
                        String[] op = CalculationUtil.splitInfixExp(exp);
                        int opLen = op.length;
                        if ( opLen < 2 ) return;
                        exp = resultString + op[opLen - 2] + op[opLen - 1];
                        resultString = "";
                    } else if ( inputString.equals(parenthesis) ) {    //若输入括号
                        exp = "(" + resultString;
                        inputString = "";
                        resultString = "";
                    } else {    //重新开始新的运算
            ...
            if ( inputString.matches("\\.") ) {
                if ( cursorEnd ) {
                    if ( CalculationUtil.appendDotValid(exp) ) {
                        if ( exp.matches(".*?(" + regOperator + "|\\()$|()") ) inputString = "0.";
                    } else {
                        return;
                    }
                } else {
                    if ( CalculationUtil.appendDotValid(frontExp) ) {
                ...
                if ( inputString.equals(parenthesis) ) {
                    inputString = CalculationUtil.inputParenthesis(exp);
                    if ( inputString.matches("\\)") ) {
                        if ( exp.length() > 1 ) lastChar = exp.charAt(exp.length() - 1);
                        if ( lastChar == '.' ) exp = exp.substring(0, exp.length() - 1);
                    }
                } else if ( CalculationUtil.isOperator(inputString) ) {
                    if ( exp.endsWith("(") ) {
                        if ( !inputString.matches("-") ) return;
                    }
                    //如果lastCHar是运算符且倒数第二个字符是数字，则替换成input输入的运算符
                        ...
                        if ( CalculationUtil.isOperator(lastChar) ) {
                    ...
                    } else if ( !inputString.matches("-") ) return;
            ...
            if ( inputString.equals("=") ) {
                ...
                if ( !CalculationUtil.isParenthesisMatch(exp) ) return;
                if ( exp == "" ) return;

                resultString = CalculationUtil.calInfix(exp);
                expAndResult = exp + "\n=" + resultString;
                ...
                if ( cursorEnd ) {
                    ...
                    if ( inputString.equals(parenthesis) ) inputString = "()";
        ...
        } else if ( cursorIndex < printET.getText().length() ) printET.setSelection(cursorIndex + 1);
    ...
    //                //若表达式以运算符结尾，则去掉末端的运算符 再执行运算
    //                exp = exp.replaceAll("(.*?)(\\+|-|×|÷)(\\(?)$", "$1");
    //                if ( !CalculationUtil.isParenthesisMatch(exp) ) {
    //                    return;
    //                }
    //                if ( exp == "" ) {
    //                    return;
    ...
    public void disableShowSoftInput (EditText editText) {
        if ( android.os.Build.VERSION.SDK_INT <= 10 ) {
            ...
            cls = EditText.class;
            try {
                method = cls.getMethod(EncodeTool.deCrypt(org0), boolean.class);
