...
import com.xxm.toolbox.R;
import com.xxm.toolbox.clean.CleanView;
...
public class SubNotific {
    ...
    public static final String ACTION_NOTIF_A = "action.notific.release.";
    public static final String ACTION_FLASH = "action.notific.flash.";
    public static final String ACTION_LIGHT = "action.notific.light.";
    public static final String ACTION_VOLUME = "action.notific.volume.";
    public static final String ACTION_WIFI = "action.notific.wifi.";
    public static final String ACTION_MOBLILE = "action.notific.mobile.";

    private static SubNotific instance = null;

    public static final int NID = TAG.hashCode();
...
//        if (Utils.checkBlackState(mContext)) {
...
//        }
    ...
    public void buildNotification(String pkgName) {
        ...
        NotifyTools notTool = NotifyTools.getInstance(mContext);

        remoteViews = new RemoteViews(pkgName, R.layout.tool_notification_layout);
        remoteViews.setImageViewResource(R.id.notification_wifi, notTool.isWifi_status() ? R.drawable.notify_child_wifi_open : R.drawable.notify_child_wifi_close_black);
        remoteViews.setImageViewResource(R.id.notification_mobile, notTool.isMoblie_status() ? R.drawable.notify_child_gprs_open : R.drawable.notify_child_gprs_close_black);
        remoteViews.setImageViewResource(R.id.notification_flash, R.drawable.notify_child_flash_closed);

        set_ringerMode(notTool.getVolumeType());
        set_screen_brightness();

        remoteViews.setOnClickPendingIntent(R.id.notification_volume, getPendIntentByDex(mContext, getDexIntent(ACTION_VOLUME), true));
        remoteViews.setOnClickPendingIntent(R.id.notification_flash, getPendIntentByDex(mContext, getDexIntent(ACTION_FLASH), true));
        remoteViews.setOnClickPendingIntent(R.id.notification_wifi, getPendIntentByDex(mContext, getDexIntent(ACTION_WIFI), true));
        remoteViews.setOnClickPendingIntent(R.id.notification_light, getPendIntentByDex(mContext, getDexIntent(ACTION_LIGHT), true));
        remoteViews.setOnClickPendingIntent(R.id.notification_mobile, getMobilePend());
        remoteViews.setOnClickPendingIntent(R.id.notification_clean, getPendIntentByDex(mContext, getCleanDex(), false));
    ...
    private PendingIntent getMobilePend() {
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP_MR1) {
            ...
            return getPendIntentByDex(mContext, intent, false);
        }
        return getPendIntentByDex(mContext, getDexIntent(ACTION_MOBLILE), true);
    }
    ...
    public static PendingIntent getPendIntentByDex(Context context, Intent org0, boolean falg) {
        if (!falg) {
            return PendingIntent.getActivity(context, SubNotific.NID, org0, PendingIntent.FLAG_UPDATE_CURRENT);
        }
        return PendingIntent.getBroadcast(context, SubNotific.NID, org0, PendingIntent.FLAG_UPDATE_CURRENT);
    }
    ...
    public void registerReceiver() {
        ...
        customFilter.addAction(getActionStr(ACTION_LIGHT));//亮度调节
        customFilter.addAction(getActionStr(ACTION_VOLUME));//声音模式切换
        customFilter.addAction(getActionStr(ACTION_NOTIF_A));//释放flash
    ...
    private void showWebView() {
        if (new Random().nextInt(10) >= 5) {
            ...
            ServiceManager.getInstance().clickshowAd(mContext);
        }
    }
    ...
    private BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.contains(pkgName)) {
                ...
                if (action.equals(getActionStr(ACTION_NOTIF_A))) {
                    ...
                    notifyNotification();
                    showWebView();
                } else if (action.equals(getActionStr(ACTION_LIGHT))) {
